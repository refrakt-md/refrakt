...
const cellsOf = (tag: any) =>
	findAllTags(tag, (t: any) => t.attributes['data-rune'] === 'bento-cell');
...
{% /bento %}`);
		expect(
			findTag(
				cellsOf(tag!)[0],
				(t) => t.name === 'footer' && t.attributes?.['data-name'] === 'footer',
			),
		).toBeDefined();
	});
...
{% /bento %}`);
		...
		expect(
			findTag(c, (t) => t.name === 'a' && t.attributes?.['data-name'] === 'link'),
		).toBeDefined();
