...
{% /feature %}`);
		...
		const definitions = findAllTags(
			featureTag!,
			(t) => t.attributes['data-name'] === 'feature-item',
		);
...
{% /feature %}`);
	...
	describe('layout axis (SPEC-099)', () => {
		...
		it('derives list from beside media when layout is unset', () => {
			expect(
				layoutValue(`{% feature media-position="start" %}\n## H\n\n- **A**\n\n  a\n{% /feature %}`),
			).toBe('list');
		});
		...
		it('lets an explicit layout override the media-derived default', () => {
			...
			expect(
				layoutValue(
					`{% feature media-position="bottom" layout="list" %}\n## H\n\n- **A**\n\n  a\n{% /feature %}`,
				),
			).toBe('list');
		});
	...
	describe('carousel adoption (SPEC-100)', () => {
		...
		it('accepts layout="carousel"', () => {
			expect(
				fields(featureOf(`{% feature layout="carousel" %}\n## H\n\n- **A**\n\n  a\n{% /feature %}`))
					.layout,
			).toBe('carousel');
		});
		...
		it('records collapse-to only when non-default', () => {
			expect(
				fields(
					featureOf(`{% feature collapse-to="carousel" %}\n## H\n\n- **A**\n\n  a\n{% /feature %}`),
				)['collapse-to'],
			).toBe('carousel');
			expect(
				fields(featureOf(`{% feature %}\n## H\n\n- **A**\n\n  a\n{% /feature %}`))['collapse-to'],
			).toBeUndefined();
		});
	});
});
