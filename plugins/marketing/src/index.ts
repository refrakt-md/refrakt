...
export const marketing: Plugin = {
	...
	runes: {
		hero: {
			transform: hero,
			description:
				'Full-width introductory section for landing pages with title, subtitle, and call-to-action',
			category: 'Section',
			snippet: [
				'{% hero align="${1|center,left,right|}" %}',
				'# ${2:Headline}',
				'',
				'${3:Subtitle or tagline}',
				'',
				'- [${4:Get Started}](${5:/docs/getting-started})',
				'{% /hero %}',
			],
...
{% /feature %}`,
		...
		steps: {
			...
			snippet: [
				...
				'## ${1:First Step}',
				'',
				'${2:Instructions for step one.}',
				'',
				'## ${3:Second Step}',
				'',
				'${4:Instructions for step two.}',
				'{% /steps %}',
			],
			fixture: `{% steps %}
...
{% /pricing %}`,
		...
		testimonial: {
			...
			aliases: ['review'],
			description:
				'Customer testimonial or review with quote, author attribution, and optional rating',
			seoType: 'Review',
			category: 'Section',
			snippet: [
				'{% testimonial %}',
				'> ${1:Quote text goes here.}',
				'',
				'**${2:Author Name}**',
				'',
				'${3:Role or company}',
...
{% /testimonial %}`,
		...
		comparison: {
			...
			aliases: ['versus', 'vs'],
			description:
				'Product/feature comparison matrix where headings become columns and bold labels align rows across columns',
			category: 'Section',
			snippet: [
				'{% comparison layout="${1:table}" %}',
				'## ${2:Option A}',
				'',
				'- **${4:Feature}**: ${5:Value}',
				'',
