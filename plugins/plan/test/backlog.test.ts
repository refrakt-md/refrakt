...
const collOf = (src: string) =>
	findTag(parse(src) as any, (t) => t.attributes['data-rune'] === 'collection');
const metaOf = (tag: any, field: string) =>
	findTag(tag, (t: any) => t.name === 'meta' && t.attributes?.['data-field'] === field)?.attributes
		?.content;
...
describe('backlog (sugar over collection)', () => {
	...
	it("a single-type backlog surfaces that type's key field; mixed stays universal", () => {
		expect(metaOf(collOf('{% backlog show="work" /%}'), 'collection-body')).toContain(
			'$item.data.priority',
		);
		expect(metaOf(collOf('{% backlog show="bug" /%}'), 'collection-body')).toContain(
			'$item.data.severity',
		);
		// mixed (work,bug) → no type-specific field, just the universal projection
		expect(metaOf(collOf('{% backlog /%}'), 'collection-body')).not.toContain(
			'$item.data.priority',
		);
	});
});
...
describe('backlog resolves through collection (WORK-342)', () => {
	...
	for (const [name, entry] of Object.entries(plan.runes)) {
		pluginRunes[name] = defineRune({
			name,
			schema: (entry as any).transform,
			aliases: (entry as any).aliases,
		});
	}
	const tags = { ...coreTags, ...runeTagMap(pluginRunes), ...Markdoc.tags } as any;
	const ctx = { info() {}, warn() {}, error() {} } as any;
	const sentiments = {
		work: { status: { done: 'positive', ready: 'neutral' } },
		bug: { status: { fixed: 'positive', confirmed: 'caution' } },
	};
	...
	const resolve = (src: string, reg: any) => {
		const tr = Markdoc.transform(Markdoc.parse(src), {
			tags,
			nodes,
			functions,
			variables: {},
	...
	const cat = (out: any) =>
		findAllTags(
			out,
			(t: any) =>
				t.attributes?.['data-rune'] === 'badge' && t.attributes?.['data-meta-type'] === 'category',
		);

	it('renders a card with a bar (identifier + sentiment-coloured status badge)', () => {
		const reg = mk([
			{ type: 'work', id: 'WORK-1', sourceUrl: '/w/1/', data: { title: 'Alpha', status: 'done' } },
		]);
		...
		const status = findTag(
			cards[0],
			(t: any) =>
				t.attributes?.['data-rune'] === 'badge' && t.attributes?.['data-meta-type'] === 'status',
		);
		...
		const reg = mk([
			{
				...
				id: '',
				sourceUrl: '/m/v1/',
				data: { name: 'v1.0.0', status: 'active' },
			},
		]);
