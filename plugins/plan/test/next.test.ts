...
describe('plan next — basic selection', () => {
	it('finds a ready work item', () => {
		writeMd(
			'work/a.md',
			`{% work id="WORK-001" status="ready" priority="high" complexity="simple" %}
# Task A
{% /work %}`,
		);
...
{% /bug %}`,
	...
	it('skips non-ready items', () => {
		writeMd(
			'work/a.md',
			`{% work id="WORK-001" status="done" priority="high" %}
# Done task
{% /work %}`,
		);
		writeMd(
			'work/b.md',
			`{% work id="WORK-002" status="in-progress" priority="high" %}
# In progress task
{% /work %}`,
		);
		writeMd(
			'work/c.md',
			`{% work id="WORK-003" status="draft" priority="high" %}
# Draft task
{% /work %}`,
		);
		const result = runNext({ dir: TMP });
...
{% /work %}`,
		...
		writeMd(
			'work/b.md',
			`{% work id="WORK-002" status="ready" priority="critical" %}
# Critical priority
{% /work %}`,
		);
		writeMd(
			'work/c.md',
			`{% work id="WORK-003" status="ready" priority="high" %}
# High priority
{% /work %}`,
		);
	...
	it('uses complexity as tiebreaker (simpler first)', () => {
		writeMd(
			'work/a.md',
			`{% work id="WORK-001" status="ready" priority="high" complexity="complex" %}
# Complex task
{% /work %}`,
		);
		writeMd(
			'work/b.md',
			`{% work id="WORK-002" status="ready" priority="high" complexity="trivial" %}
# Trivial task
{% /work %}`,
		);
...
describe('plan next — dependency exclusion', () => {
	it('excludes items with unfinished dependencies', () => {
		writeMd(
			'work/dep.md',
			`{% work id="WORK-001" status="in-progress" priority="high" %}
# Dependency (not done)
{% /work %}`,
		);
		writeMd(
...
{% /work %}`,
		...
		writeMd(
			'work/free.md',
			`{% work id="WORK-003" status="ready" priority="low" %}
# No dependencies
{% /work %}`,
		);
		const result = runNext({ dir: TMP });
...
describe('plan next — filters', () => {
	beforeEach(() => {
		writeMd(
			'work/a.md',
			`{% work id="WORK-001" status="ready" priority="high" milestone="v1.0" tags="cli, plan" assignee="alice" %}
# Task A
{% /work %}`,
		);
		writeMd(
			'work/b.md',
			`{% work id="WORK-002" status="ready" priority="medium" milestone="v2.0" tags="docs" assignee="bob" %}
# Task B
{% /work %}`,
		);
		writeMd(
			'bug/c.md',
			`{% bug id="BUG-001" status="confirmed" severity="major" tags="cli" %}
# Bug C
{% /bug %}`,
		);
	});
...
{% /work %}`,
		...
		writeMd(
			'work/b.md',
			`{% work id="WORK-002" status="ready" priority="medium" %}
# B
{% /work %}`,
		);
		writeMd(
...
describe('plan next — JSON output includes full data', () => {
	it('includes criteria and refs in result', () => {
		writeMd(
			'work/a.md',
			`{% work id="WORK-001" status="ready" priority="high" complexity="moderate" %}
# Full task
