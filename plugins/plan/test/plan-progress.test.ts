...
const headingsOf = (src: string) => {
	...
	return findAllTags(
		wrap,
		(t: any) => t.name === 'h3' && t.attributes?.['data-name'] === 'heading',
	);
};
...
describe('plan-progress (per-type aggregate composition)', () => {
	...
	it('labels each progress bar with the achieved-status word (no numbers in the label)', () => {
		const aggs = aggsOf('{% plan-progress /%}');
		expect(metaOf(aggs[0], 'aggregate-body')).toContain(
...
describe('plan-progress resolves to per-type bars + status badges', () => {
	...
	const resolved = (() => {
		const transformed = Markdoc.transform(Markdoc.parse('{% plan-progress /%}'), {
			tags,
			nodes,
			functions,
			variables: {},
		...
		return resolveAggregates(
			...
			'/p/',
			reg,
			{ tags, nodes, functions, sentiments } as any,
			ctx,
		);
	})();
	...
	it('renders per-type status badges (work: done×2 + ready×1)', () => {
		...
		expect(
			badges.find((b) => textOf(b).includes('Done'))?.attributes?.['data-meta-sentiment'],
		).toBe('positive');
	});

	it('keeps the type heading above each block', () => {
		const headings = findAllTags(
			resolved as any,
			(t) => t.name === 'h3' && t.attributes?.['data-name'] === 'heading',
		);
		expect(headings.length).toBe(2);
	});
});
