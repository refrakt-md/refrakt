...
beforeEach(() => {
	root = join(
		tmpdir(),
		`refrakt-plan-embed-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
	);
	...
	writeFileSync(
		join(root, 'plan', 'specs', 'SPEC-001-foo.md'),
		`{% spec id="SPEC-001" status="accepted" %}\n\n# Foo spec\n\nBody.\n\n{% /spec %}\n`,
	);
	writeFileSync(
		join(root, 'plan', 'work', 'WORK-001-bar.md'),
		`{% work id="WORK-001" status="ready" %}\n\n# Bar work\n\n## Acceptance Criteria\n- [ ] one\n\n{% /work %}\n`,
	);
	writeFileSync(
		join(root, 'plan', 'milestones', 'v1.0.0.md'),
