...
export const places: Plugin = {
	...
	runes: {
		event: {
			...
			category: 'Semantic',
			snippet: [
				'{% event date="${1:${CURRENT_YEAR}-${CURRENT_MONTH}-${CURRENT_DATE}}" location="${2:Location}" %}',
				'# ${3:Event Name}',
				'',
				'${4:Event description}',
				'{% /event %}',
...
{% /event %}`,
		...
		map: {
			transform: map,
			description:
				'Interactive map visualization from Markdown lists of locations with pins, routes, and grouped layers',
			seoType: 'Place',
			category: 'Semantic',
			snippet: [
				'{% map zoom=${1:13} %}',
				'## ${2:Landmarks}',
				'',
				'- **${3:Place Name}** \\u2014 *${4:Description}* \\u2014 ${5:48.8566}, ${6:2.3522}',
				'{% /map %}',
			],
...
{% /map %}`,
		...
		itinerary: {
			...
			description:
				'Travel itinerary with day groupings and timed stops. Headings become stops with "time — location" parsing; h2 headings create day groups.',
			seoType: 'ItemList',
			category: 'Semantic',
			snippet: [
				'{% itinerary %}',
...
{% /itinerary %}`,
		...
		'itinerary-stop': {
			...
			description:
				'Individual stop within an itinerary with time, location, and optional duration/activity',
		},
	},
	theme: {
