...
- [ ] Scaffold writes `AGENTS.md` with the expected rune reference (runs automatically during scaffold; confirm the file exists and lists the runes from the configured packages)
...
Scaffolded sites ship with an `AGENTS.md` file — the canonical rune reference for human contributors and coding agents. It's generated by `refrakt reference dump` and must be regenerated when:

- Packages are added or removed in `refrakt.config.json`
- An `@refrakt-md/*` package upgrade changes the rune set

Sites can catch drift in CI by running:

```bash
npx refrakt reference dump --check
```

Exit code is non-zero if the checked-in `AGENTS.md` differs from what the current config would produce. Recommend adding this to downstream site CI pipelines when documenting the scaffold.
