...
export class RfDiagram extends SafeHTMLElement {
	...
	private async renderMermaid(source: string, container: HTMLElement) {
		...
		mermaid.initialize({
			...
			theme: 'base',
			themeVariables: {
				primaryColor: style.getPropertyValue('--rf-color-primary-100').trim() || '#f5f4f1',
				primaryTextColor: style.getPropertyValue('--rf-color-text').trim() || '#1c1a17',
				primaryBorderColor: style.getPropertyValue('--rf-color-primary-400').trim() || '#bfbab2',
				...
				mainBkg: style.getPropertyValue('--rf-color-primary-100').trim() || '#f5f4f1',
				nodeBorder: style.getPropertyValue('--rf-color-primary-400').trim() || '#bfbab2',
