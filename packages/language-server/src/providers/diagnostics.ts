import { Diagnostic, DiagnosticSeverity, type TextDocuments } from 'vscode-languageserver';
import type { TextDocument } from 'vscode-languageserver-textdocument';
import { validate } from '../parser/markdoc.js';
import { findSimilar, hasPartial, getPartialNames } from '../registry/loader.js';

export function provideDiagnostics(document: TextDocument): Diagnostic[] {
...
function checkPartialReferences(text: string): Diagnostic[] {
	...
	while ((match = partialRegex.exec(text)) !== null) {
		...
		if (!hasPartial(fileName)) {
			...
			const similar = allPartials.filter((p) => levenshteinDistance(fileName, p) <= 3).slice(0, 3);
			if (similar.length > 0) {
