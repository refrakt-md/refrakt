		...
		if (node.children && node.children.length > 0) {
			summary.children = node.children.map((child) => {
				if (Tag.isTag(child)) {
					...
					if (child.attributes?.['data-rune'])
						childSummary['data-rune'] = child.attributes['data-rune'];
					if (child.attributes?.['data-field'])
						childSummary['data-field'] = child.attributes['data-field'];
					if (child.attributes?.['data-name'])
						childSummary['data-name'] = child.attributes['data-name'];
					return childSummary;
				}
 ...
 */
function loadThemeTransform(workspaceRoot: string): {
	transform: ((tree: any) => any) | null;
	error?: string;
} {
	try {
		...
		const themePackage =
			typeof config.theme === 'string'
				...
				: typeof config.theme === 'object' &&
						config.theme !== null &&
						typeof config.theme.package === 'string'
					? config.theme.package
					: null;
		if (!themePackage) {
			return {
				transform: null,
				error: '"theme" must be a string package name or { package, ... } object',
			};
		}
...
export function inspectRuneAtPosition(
	...
	const matchedRenderable =
		runeIndex >= 0 && runeIndex < allRenderables.length
