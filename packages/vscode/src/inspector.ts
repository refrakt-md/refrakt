...
function buildObjectTree(obj: unknown, parentLabel?: string): InspectorNode[] {
	...
	if (Array.isArray(obj)) {
		return obj.map((item, i) => {
			...
			const children = typeof item === 'object' && item !== null ? buildObjectChildren(item) : [];
...
export class RuneInspectorProvider implements vscode.TreeDataProvider<InspectorNode> {
	...
	private buildRootNodes(result: InspectorResult): InspectorNode[] {
		...
		const astNode = new InspectorNode(
			'AST',
			result.runeName,
			astChildren,
			result.stages.ast,
		...
		// Transform stage
		const transformChildren = buildObjectChildren(
			result.stages.transform as Record<string, unknown>,
		);
		const transformNode = new InspectorNode(
			'Transform',
			'',
			transformChildren,
		...
		// Serialized stage
		const serializedChildren = buildObjectChildren(
			result.stages.serialized as Record<string, unknown>,
		);
		const serializedNode = new InspectorNode(
			'Serialized',
			'',
			serializedChildren,
		...
		if (result.stages.identity) {
			const identityChildren = buildObjectChildren(
				result.stages.identity as Record<string, unknown>,
			);
			const identityNode = new InspectorNode(
				'Identity Transform',
				'',
				identityChildren,
				result.stages.identity,
				'identity',
			);
