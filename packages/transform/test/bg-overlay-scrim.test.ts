...
describe('SPEC-088 overlay vocabulary', () => {
	it('dark/light → data-bg-overlay flag', () => {
		...
		const o = find(
			asTag(t(makeTag('section', { 'data-rune': 'hero' }, [meta('bg-overlay', 'dark')]))),
			'bg-overlay',
		)!;
	...
	it('a token reference → background var(--rf-color-*) wash with opacity', () => {
		...
		const o = find(
			asTag(
				t(
					makeTag('section', { 'data-rune': 'hero' }, [
						meta('bg-overlay', 'primary'),
						meta('bg-overlay-opacity', '0.4'),
					]),
				),
			),
			'bg-overlay',
	...
	it('raw CSS still applies but warns (deprecated)', () => {
		...
		const o = find(
			asTag(
				t(makeTag('section', { 'data-rune': 'hero' }, [meta('bg-overlay', 'rgba(0,0,0,0.3)')])),
			),
			'bg-overlay',
		)!;
...
describe('SPEC-088 scrim', () => {
	...
	it('consumes scrim metas', () => {
		...
		const leaked = (r.children as any[]).some(
			(c) => c?.name === 'meta' && c.attributes?.['data-field'] === 'scrim',
		);
