...
describe('buildGithubBlobUrl (SPEC-078)', () => {
	...
	it('defaults repoBranch to "main" when omitted', () => {
		expect(buildGithubBlobUrl(repo, undefined, 'packages/types/src/theme.ts')).toBe(
			`${repo}/blob/main/packages/types/src/theme.ts`,
		);
		expect(buildGithubBlobUrl(repo, '', 'packages/types/src/theme.ts')).toBe(
			`${repo}/blob/main/packages/types/src/theme.ts`,
		);
	});

	it('accepts a tag as the ref', () => {
		expect(buildGithubBlobUrl(repo, 'v0.16.0', 'packages/types/src/theme.ts')).toBe(
			`${repo}/blob/v0.16.0/packages/types/src/theme.ts`,
		);
	});

	it('accepts a commit SHA as the ref (archival URLs)', () => {
		expect(buildGithubBlobUrl(repo, 'a1b2c3d4', 'packages/types/src/theme.ts')).toBe(
			`${repo}/blob/a1b2c3d4/packages/types/src/theme.ts`,
		);
	});

	it('appends a single-line anchor when lines is a single number', () => {
		expect(buildGithubBlobUrl(repo, 'main', 'foo.ts', '42')).toBe(`${repo}/blob/main/foo.ts#L42`);
	});

	it('appends a range anchor when lines is start-end', () => {
		expect(buildGithubBlobUrl(repo, 'main', 'foo.ts', '42-58')).toBe(
			`${repo}/blob/main/foo.ts#L42-L58`,
		);
	});
	...
	it('strips trailing slash on repoUrl', () => {
		expect(buildGithubBlobUrl(`${repo}/`, 'main', 'foo.ts')).toBe(`${repo}/blob/main/foo.ts`);
	});

	it('percent-encodes special characters in path segments but preserves /', () => {
		expect(buildGithubBlobUrl(repo, 'main', 'docs/My Folder/foo (bar).ts')).toBe(
			`${repo}/blob/main/docs/My%20Folder/foo%20(bar).ts`,
		);
	});
