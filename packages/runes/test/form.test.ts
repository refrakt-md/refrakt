...
{% /form %}`);
		...
		const field = findTag(result as any, (t) => t.attributes['data-rune'] === 'form-field');
		const requiredSpan = findTag(
			field!,
			(t) => t.name === 'span' && t.attributes['data-name'] === 'required',
		);
...
{% /form %}`);
		...
		const form = findTag(result as any, (t) => t.attributes['data-rune'] === 'form');
		const hr = findTag(form!, (t) => t.name === 'hr' && t.attributes['data-name'] === 'separator');
...
{% /form %}`);
		...
		const form = findTag(result as any, (t) => t.attributes['data-rune'] === 'form');
		const help = findTag(form!, (t) => t.name === 'p' && t.attributes['data-name'] === 'help');
...
{% /form %}`);
		...
		const form = findTag(result as any, (t) => t.attributes['data-rune'] === 'form');
		const desc = findTag(form!, (t) => t.name === 'p' && t.attributes['data-name'] === 'text');
...
{% /form %}`);
		...
		const form = findTag(result as any, (t) => t.attributes['data-rune'] === 'form');
		const fieldset = findTag(
			form!,
			(t) => t.name === 'fieldset' && t.attributes.class === 'rf-form-fieldset',
		);
...
{% /form %}`);
		...
		const fieldset = findTag(
			form!,
			(t) => t.name === 'fieldset' && t.attributes['data-rune'] === 'form-field',
		);
		expect(fieldset).toBeDefined();

		const inputs = findAllTags(
			fieldset!,
			(t) => t.name === 'input' && t.attributes.type === 'radio',
		);
...
{% /form %}`);
		...
		const inputs = findAllTags(
			fieldset!,
			(t) => t.name === 'input' && t.attributes.type === 'checkbox',
		);
		expect(inputs.length).toBe(3);
	});
