...
function makeSpecEntity(
	id: string,
	sourceFile: string,
	runeType: string = 'spec',
): EntityRegistration {
	return {
		type: runeType,
		id,
...
describe('expand pipeline (SPEC-066, WORK-260)', () => {
	beforeEach(() => {
		projectRoot = join(
			tmpdir(),
			`refrakt-expand-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
		);
...
`,
			...
			const link = findTag(
				...
				(t) =>
					t.name === 'a' &&
					(t.attributes.class as string | undefined)?.includes('rf-expand__canonical-link'),
			);
...
`,
			...
			const placeholder = parse(
				`{% expand "SPEC-001" canonical=true label="Read the full spec" /%}`,
			);
			const { ctx } = makeCtx();
			const resolved = resolveExpands(placeholder, '/page/', registry, [], embedConfig(), ctx);
			const link = findTag(
