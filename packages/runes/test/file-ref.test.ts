...
describe('file-ref rune (SPEC-078)', () => {
	describe('without preview', () => {
		it('renders an inline anchor with the resolved GitHub URL', () => {
			const tree = render(
				'See {% file-ref path="packages/types/src/token-contract.ts" lines="42-58" label="ThemeTokensConfig" /%}.',
			);
			const { ctx } = makeCtx();
			const resolved = resolveFileRefs(tree, '/p/', REPO, 'main', ctx);
			const anchors = findAll(
				resolved,
				(t) => t.name === 'a' && (t.attributes.href as string | undefined)?.includes('github.com'),
			);
		...
		it('per-page repoUrl warning fires at most once even with many file-refs', () => {
			const tree = render(
				'{% file-ref path="a.ts" /%} and {% file-ref path="b.ts" /%} and {% file-ref path="c.ts" /%}.',
			);
			const { ctx, messages } = makeCtx();
		...
		it('supports tag and SHA refs via repoBranch', () => {
			...
			const anchor = findAll(
				resolved,
				(t) => t.name === 'a' && (t.attributes.href as string | undefined)?.startsWith(REPO),
			)[0];
	...
	describe('with preview="drawer"', () => {
		it('emits a hoist sentinel + the inline anchor links to the drawer fragment', () => {
			const tree = render(
				'See {% file-ref path="foo.ts" lines="1-10" label="x" preview="drawer" /%}.',
			);
			const { ctx } = makeCtx();
			const resolved = resolveFileRefs(tree, '/p/', REPO, 'main', ctx);
		...
		it('builds a hoisted drawer with snippet body + GitHub footer when repoUrl + projectRoot resolve', () => {
			...
			resolved = hoistPreviewDrawers(
				resolved,
				'/p/',
				undefined,
				fsProjectFiles(process.cwd()),
				ctx,
			);
			...
			expect((footerLinks[0].children?.[0] as string).startsWith('View source on GitHub')).toBe(
				true,
			);
		});
		...
		it('multiple references to the same path+lines hoist a single drawer', () => {
			const tree = render(
				'A {% file-ref path="package.json" preview="drawer" label="one" /%} and B {% file-ref path="package.json" preview="drawer" label="two" /%}.',
			);
			const { ctx } = makeCtx();
			let resolved = resolveFileRefs(tree, '/p/', REPO, 'main', ctx);
			resolved = hoistPreviewDrawers(
			...
			const anchors = findAll(
				...
				(t) =>
					...
					(t.attributes.href as string).startsWith('#drawer-'),
			);
