...
describe('formatter functions', () => {
	it('currency formats a number with a currency code', () => {
		...
		expect(functions.currency.transform!({ 0: 1234.5, 1: 'EUR' } as never, {} as never)).toBe(
			'€1,234.50',
		);
	});

	it('date formats an ISO date', () => {
		expect(functions.date.transform!({ 0: '2024-01-15' } as never, {} as never)).toBe(
			'Jan 15, 2024',
		);
	});
	...
	it('join joins arrays with a separator', () => {
		expect(functions.join.transform!({ 0: ['a', 'b', 'c'], 1: ' · ' } as never, {} as never)).toBe(
			'a · b · c',
		);
	});
	...
	it('concat stringifies and joins positional args (no separator)', () => {
		expect(
			functions.concat.transform!({ 0: 'milestone:', 1: 'v0.16.0' } as never, {} as never),
		).toBe('milestone:v0.16.0');
		expect(functions.concat.transform!({ 0: 'a', 1: 'b', 2: 'c' } as never, {} as never)).toBe(
			'abc',
		);
		expect(functions.concat.transform!({ 0: 'n=', 1: 42 } as never, {} as never)).toBe('n=42');
		expect(functions.concat.transform!({ 0: 'x:', 1: null, 2: 'y' } as never, {} as never)).toBe(
			'x:y',
		);
	});
	...
	it('humanize is exposed as a markdoc function', () => {
		expect(functions.humanize.transform!({ 0: 'blocked-by' } as never, {} as never)).toBe(
			'Blocked By',
		);
