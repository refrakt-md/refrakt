...
- 550dc77: Remove the `error` rune (WORK-555)
...
- 2c1b5c2: Give `{% data %}` an optional per-row body (SPEC-127)
...
- fde9ae0: Carry universal attribute records in `serializeRune`, and filter internal attributes by prefix
...
- 7532b55: `{% accordion schema="none" %}` emits no structured data (WORK-552)
  ...
  `ItemList` would need `itemListElement` on the root _and_ `ListItem` with
...
- ec7357b: Add `{% code %}`, an inline code span that can hold a variable (WORK-551)
...
- ec7357b: `{% data %}`: a body can now emit a table **row** (WORK-550)
...
- bc06abe: `{% data %}`: an empty filter result is no longer a build error (BUG-011)
...
- f23d794: `{% data %}`: an attribute the rune cannot read is an error, not an empty string (BUG-010)
...
- 9f912a1: Fix heading IDs dropping inline code and keeping punctuation (BUG-005)
...
- a6752f4: `{% data %}`: a query can now nest inside another query's body (WORK-553)
...
- 0d3ebed: Rune attribute tables are generated from the schemas (WORK-548, SPEC-128)
...
- cf0489f: Page frontmatter is now indexed onto the `page` entity, so `collection` and `aggregate` can filter and group pages by any frontmatter field — `tags`, `author`, `image`, or your own custom fields (e.g. `category`, `status`). Routing/render-control keys (`layout`, `tint`, `tint-mode`, `tint-lock`, `slug`, `redirect`) are excluded so they don't pollute queries; the curated fields (`title`, `description`, `date`, `order`, `icon`, `draft`) keep their normalised values. Enables tag-driven page collections and registry-fed catalogues (SPEC-092 Layer 1).
...
- b8d9396: Fix the `tree` shape of a data-bound sandbox: page entity `url`s carry no trailing slash while `parentUrl`s do, so the nesting never matched and the tree came out flat. Normalize trailing slashes when building the tree so parent↔child relationships resolve.
