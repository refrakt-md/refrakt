...
export const grid = createContentModelSchema({
	attributes: {
		...
		flow: { type: String, required: false, matches: flow.slice() },
		spans: { type: SpaceSeparatedList, required: false },
		ratio: { type: String, required: false },
		gap: { type: String, required: false, matches: ['none', 'tight', 'default', 'loose'] },
		valign: { type: String, required: false, matches: ['top', 'center', 'bottom', 'stretch', 'baseline'] },
		collapse: { type: String, required: false, matches: ['sm', 'md', 'lg', 'never'] },
		mode: { type: String, required: false, matches: ['columns', 'auto', 'masonry'], default: 'columns' },
		min: { type: String, required: false },
		aspect: { type: String, required: false },
		stack: { type: String, required: false, matches: ['natural', 'reverse'] },
	},
	contentModel: {
		type: 'delimited',
		delimiter: 'hr',
