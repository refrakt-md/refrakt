...
export const relationships = createContentModelSchema({
	attributes: {
		of: {
			...
			required: false,
			default: '',
			description: 'Entity to describe — an id (e.g. `of=$item.id`) or a bound entity.',
		},
		kind: {
			type: String,
			required: false,
			default: '',
			description: 'Edge kinds to include, comma-separated (e.g. "blocks,blocked-by").',
		},
		type: {
			type: String,
			required: false,
			default: '',
			description: 'Restrict related entity types, comma-separated.',
		},
		group: {
			type: String,
			required: false,
			default: 'kind',
			description: 'Group-by: kind (default) | type | none.',
		},
		'group-display': {
			type: String,
			required: false,
			...
			description:
				'How groups are presented: headings (default) or accordion (collapsible native <details> panels, styled like the accordion rune, with a per-group count).',
		},
		sort: {
			type: String,
			required: false,
			default: '',
			description: 'Sort related entities by a field (prefix - for descending).',
		},
		...
		fields: {
			...
			required: false,
			default: '',
			description: 'Comma-separated data fields to project in the no-body built-in.',
		},
		layout: {
			type: String,
			required: false,
			default: 'list',
			description:
				'Arrangement: list (stacked) | grid (multi-column). Item chrome comes from the item — the no-body built-in, or a rune like {% card %} in the body template.',
		},
		empty: {
			type: String,
			required: false,
			default: '',
			description:
				'Fallback text shown when there are no edges (no-body form; body form uses a fallback zone). Absent → render nothing.',
		},
	},
	deferBody: true,
