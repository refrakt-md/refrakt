...
function convertConversationChildren(nodes: Node[], attrs: Record<string, unknown>): Node[] {
	...
	for (const node of nodes) {
		if (node.type === 'blockquote') {
			...
			const firstPara =
				children.length > 0 && children[0].type === 'paragraph' ? children[0] : null;
			const inlineHost =
				firstPara && firstPara.children[0]?.type === 'inline' ? firstPara.children[0] : firstPara;
...
export const conversation = createContentModelSchema({
	attributes: {
		speakers: {
			type: String,
			required: false,
			description: 'Comma-separated speaker names for alternating messages',
		},
	},
	contentModel: {
		type: 'custom',
		...
		description:
			'Converts blockquotes to conversation messages with alternating left/right alignment. ' +
