...
export const chart = createContentModelSchema({
	attributes: {
		...
		stacked: {
			type: Boolean,
			required: false,
			description: 'Stack data series instead of grouping',
		},
		'tick-count': {
			type: Number,
			required: false,
			description: 'Approximate number of Y-axis ticks (default 5). Ignored when tick-step is set.',
		},
		'tick-step': {
			type: Number,
			required: false,
			description: 'Explicit unit-span between Y-axis ticks (e.g. 10, 0.5). Overrides tick-count.',
		},
		'label-angle': {
			type: String,
			required: false,
	...
	transform(resolved, attrs, config) {
		...
		const tickCountMeta =
			tickCount != null ? new Tag('meta', { content: String(tickCount) }) : undefined;
		const tickStepMeta =
			tickStep != null ? new Tag('meta', { content: String(tickStep) }) : undefined;
		const labelAngleMeta =
			labelAngle != null && labelAngle !== ''
				? new Tag('meta', { content: String(labelAngle) })
				: undefined;
