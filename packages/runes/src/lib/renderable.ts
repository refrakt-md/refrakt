...
export class RenderableNodeCursor<T extends RenderableTreeNode = RenderableTreeNode> {
	...
	wrap<TagName extends string>(
		tag: TagName,
		attributes: Record<string, any> = {},
	): RenderableNodeCursor<Tag<TagName>> {
	...
	tag<TagName extends NodeType>(tag: TagName): RenderableNodeCursor<Tag<TagName>> {
		const nodes = this.nodes.filter(
			(n) => Markdoc.Tag.isTag(n) && n.name === tag,
		) as unknown as Tag<TagName>[];
		return new RenderableNodeCursor(nodes);
	}

	tags<TagNames extends NodeType[]>(
		...tags: TagNames
	): RenderableNodeCursor<Tag<TagNames[number]>> {
		const nodes = this.nodes.filter(
			(n) => Markdoc.Tag.isTag(n) && (tags as string[]).includes(n.name),
		) as unknown as Tag<TagNames[number]>[];
		return new RenderableNodeCursor(nodes);
	}
	...
	typeof(type: string): RenderableNodeCursor<T> {
		return new RenderableNodeCursor(
			this.nodes.filter(
				(n) => Markdoc.Tag.isTag(n) && n.attributes['data-rune'] === toKebabCase(type),
			),
		);
	}
