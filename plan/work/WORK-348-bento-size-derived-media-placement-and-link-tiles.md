...
- [ ] **Automatic progressive reduction.** Between full grid and stacked, the column count steps down responsively and every span **auto-caps via `min(span, current-columns)`** — no per-size `!important` overrides — so wide cells degrade gracefully at each step. (Per-cell responsive spans are deferred — {% ref "WORK-354" /%}.)
