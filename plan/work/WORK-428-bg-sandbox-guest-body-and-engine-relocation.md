{% work id="WORK-428" status="in-progress" priority="high" complexity="complex" source="SPEC-104" milestone="v0.24.0" tags="surfaces,bg,sandbox,engine,transform,lumina" %}
...
- [ ] `bg` accepts an optional body holding one bare guest (`sandbox`); bg transforms it (real sandbox rune — file resolution + sanitisation), tags it `data-bg-guest`, and forces `height="fill"` + the backdrop posture.
...
- [ ] **Reduced motion & performance:** under `prefers-reduced-motion` the live scene is not mounted (boot frame stands in); the backdrop suspends off-screen / on a hidden tab; no-JS/crawler render the boot frame.
- [ ] Unit tests cover the body transform + tagging, the engine relocation + consumption, the guardrail warning, the boot-frame layering, and the backdrop-vs-presentational mount distinction; contracts regenerated.
