...
Every refrakt plugin that registers entities into the `EntityRegistry` already has the data structure that would support inline embedding — title, status, source location, type. What's missing is a generic transclusion mechanism that consults the registry, fetches the source, and substitutes it inline with cross-cutting affordances (heading demotion, cycle detection, canonical-link).
...
{# With explicit demotion level — embed sits inside an existing section #}
...
The embedded content's heading levels are demoted by `level - 1`:
...
All headings in the extracted subtree shift by the same delta — hierarchy is preserved, just translated.

### Clamp at H6

Any heading that would become H7 or deeper is clamped to H6, with a build warning naming the source location and the affected heading text:

```
Warning: expand "SPEC-023" at site/content/blog/example.md:24 — heading
demotion would push 2 heading(s) past H6. Clamped to H6:
  - "Acceptance criteria" (was H5, would be H7)
  - "Out of scope" (was H5, would be H7)

Consider reducing level= or restructuring the source's heading depth.
```

The warning is intentionally noisy because heading hierarchy past H6 indicates a real structural mismatch worth surfacing.

### What gets demoted

Only `heading` nodes in the standard Markdoc node tree. Rune-emitted custom elements that happen to be styled as headings (e.g. a plan-rune's title element if implemented as a non-`heading` element) are not affected — they remain at whatever level the rune emits. This is by design: rune-internal heading semantics are the rune's concern, not expand's.
...
- [ ] `level=N` attribute (default 2) demotes all headings in the embedded subtree by `level - 1`
- [ ] Heading demotion preserves relative hierarchy
- [ ] Headings clamped at H6 emit a build warning naming source location and affected heading text
...
- [ ] Authoring docs cover the rune, the level= attribute, heading demotion behavior, and the relationship to xref / partial / embed
