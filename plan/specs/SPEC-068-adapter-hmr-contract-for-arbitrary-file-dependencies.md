...
- `{% code-file path="..." /%}` from {% ref "SPEC-062" /%} reads arbitrary project-root-relative files at build time.
...
- Editing an embedded source file (`src/lib/foo.ts` referenced via `code-file`) doesn't trigger a rebuild of the host page in dev.
...
**Shared with security sandbox.** The watcher must respect the same path-validation as the code-file resolver — no watching files outside the project root, no following symlinks that escape, no permission-elevation surprises. Resolver and watcher should share the validation code.

**Granularity decisions worth thinking through:**

- File-level watch for `code-file` (specific paths referenced).
...
- Heavy code-file users (API-docs site embedding source files all over): hundreds of files; still well under OS limits.
...
- What's the right developer signal when a dependency changes outside the watcher's known set (e.g., a code-file reference added after the dev server started)? Auto-add to the watcher on next build cycle?
...
- {% ref "SPEC-062" /%} — code-file rune (the canonical "read arbitrary project file" consumer)
