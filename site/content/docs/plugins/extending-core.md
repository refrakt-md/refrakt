...
description: How to add attributes to existing core runes from a plugin
---
...
The `extends` field on a `Plugin` maps core rune names to extension definitions:
...
Extensions do **not** alter the rune's Markdoc transform logic. If you need to change how the rune processes its children or what it outputs, you need to define a new rune (see [Building a Custom Plugin](authoring)) or override the core rune using `runes.prefer`.
