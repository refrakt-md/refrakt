...
The engine. Provides `createTransform(config)` which returns the identity transform function. Also defines the TypeScript interfaces (`ThemeConfig`, `RuneConfig`, `StructureEntry`) that all themes implement.
