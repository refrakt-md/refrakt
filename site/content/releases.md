...
- `bindRow` in `data-pipeline.ts` cloned nodes without carrying `inline` across — under a comment that claimed it copied exactly that. 6,609 findings; fixed here, along with the same omission in `cloneNode`. than self-closing, so the ref swallowed the rest of the paragraph. `caution | check | note | warning`, so it rendered `data-type="tip"` with no CSS behind it.
...
- (`[data-displace="both"]`). The page rendered correctly and the schema refused it; nothing else in the toolchain compares the two. `limit=5` and `limit=20`, and the resolver reads it as `Number(limitRaw)`. Now `Number`, matching the plan plugin's `backlog` and `plan-activity`. it produced no finding; corrected anyway.
...
- they are read from, its closed vocabulary where the engine owns one, the data attributes, custom properties and elements it emits, and the condition under which it applies. Selector patterns carry a `{block}` placeholder. resolved defaults, the surface an axis lands on, the block-substituted selectors it adds (`.rf-card--tinted`, `.rf-card--has-bg`), and, under `unavailable`, the axes its config rules out with the reason.
...
- closed set is enforced at parse time by the schema's `matches`, not by the engine. module-level `*_WARNED` sets they will replace). Narrowing that scope is a separate, deliberate decision under WORK-524.
...
- reads the theme's frame registry; `tints` and `backgrounds` are declared alongside it for the tint and background axes that follow. its own `postAssemble`, for resolved work that is neither emitted nor expressible as a string. The alternative is re-resolving in the second phase, which duplicates work and lets the two phases disagree. chrome that finds no media zone is only detectable against the assembled tree, and routing that through the console directly would bypass the collector's dedupe and make it untestable.
...
- resolved register, which worked because the two blocks happened to sit five lines apart. It is now a registry constraint the driver enforces. `modifierValues` key insertion order and inline-style declaration order are all part of the output, and all now follow from one ordered list rather than from where statements happen to fall in an 800-line function.
...
- render-scoped `LocaleContext` with `resolveLocaleString` / `resolvePluralString` (CLDR plurals via `Intl.PluralRules`) and per-key first-match fallback with BCP-47 region stripping (`de-AT`→`de`). (optional `i18nKey` override); covers structure/meta-field labels, layout chrome, computed navigation, programmatic text (`data-i18n`), and enum-as-text values. key→English dictionary as JSON; `--check` reports per-locale coverage and fails on drift. `<script id="rf-strings">`), resolved element→block→English synchronously with no fetch and no flash-of-English. under site overrides (site wins). seeded German translation; partial bundles degrade per key to English.
